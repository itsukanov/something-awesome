...
trait ProblemsSimulator {
  def simulateProblems[T, F[_]](ft: F[T])(implicit ME: MonadError[F, Throwable], T: Timer[F]): F[T]
}
...
object HappyPath extends ProblemsSimulator {
  override def simulateProblems[T, F[_]](ft: F[T])(implicit ME: MonadError[F, Throwable], T: Timer[F]) = ft
}
...
class CombinedSimulator(weight2scenario: Seq[(Int, ProblemsSimulator)]) extends ProblemsSimulator {
  ...
  private val scenarios = weight2scenario.flatMap {
    case (weight, scenario) => Seq.fill(weight)(scenario)
  }
